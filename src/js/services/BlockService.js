...
const transformBlock = block => {
    if (block.totalFee) {
        block.totalFee = Money.fromCoins(block.totalFee, Currency.FFT);
    }

    if (block.reward) {
        block.reward = Money.fromCoins(block.reward, Currency.FFT);
    }

    if (block.desiredReward) {
        block.desiredReward = Money.fromCoins(block.desiredReward, Currency.FFT);
    }
