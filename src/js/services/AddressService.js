...
export class AddressService extends ApiClientService {
    ...
    loadBalance = (address) => {
        return this.getApi().addresses.details(address).then(balanceResponse => {
            ...
            return {
                regular: Money.fromCoins(data.regular, Currency.FFT).toString(),
                generating: Money.fromCoins(data.generating, Currency.FFT).toString(),
                available: Money.fromCoins(data.available, Currency.FFT).toString(),
                effective: Money.fromCoins(data.effective, Currency.FFT).toString()
            };
        });
    };
